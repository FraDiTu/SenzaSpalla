...
// Metodo createNavigationMenu aggiornato per includere la Bacheca
private VBox createNavigationMenu() {
    ...
    // Bacheca pubblica per tutti
    addMenuButton(menu, "📋 Bacheca Menu", this::showBacheca);
    menu.getChildren().add(new Separator());
    ...
    if (currentUser instanceof Chef) {
        ...
        addMenuButton(menu, "I Miei Eventi", this::showEventiChef);
...
// Nuovo metodo per mostrare la bacheca
private void showBacheca() {
    BachecaView bachecaView = new BachecaView();
    root.setCenter(bachecaView.getView());
}

// Metodo per eventi chef (già definito precedentemente)
private void showEventiChef() {
    EventiChefView eventiChefView = new EventiChefView();
    root.setCenter(eventiChefView.getView());
}
