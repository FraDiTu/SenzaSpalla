// File: src/main/java/it/catering/catring/view/MieiCompitiView.java
...
public class MieiCompitiView {
    ...
    private HBox createActionButtons() {
        ...
        // Bind dei bottoni allo stato del compito selezionato
        compitiTable.getSelectionModel().selectedItemProperty().addListener((obs, oldCompito, newCompito) -> {
            boolean hasSelection = newCompito != null;
            startButton.setDisable(!hasSelection || (hasSelection && newCompito.getStato() != StatoCompito.ASSEGNATO));
            completeButton.setDisable(!hasSelection || (hasSelection && newCompito.getStato() != StatoCompito.IN_CORSO));
            problemButton.setDisable(!hasSelection || (hasSelection && newCompito.getStato() == StatoCompito.COMPLETATO));
        });
        
        // Inizializza lo stato dei bottoni
        startButton.setDisable(true);
        completeButton.setDisable(true);
        problemButton.setDisable(true);
    ...
    private void iniziaCompito() {
        ...
        try {
            ...
            // Aggiorna immediatamente la tabella
            Platform.runLater(() -> {
                compitiTable.refresh();
                loadMieiCompiti();
            });
    ...
    private void segnalaProblema() {
        ...
        ButtonType segnalaButtonType = new ButtonType("Segnala", ButtonBar.ButtonData.OK_DONE);
        dialog.getDialogPane().getButtonTypes().addAll(segnalaButtonType, ButtonType.CANCEL);
        
        TextArea noteArea = new TextArea();
        noteArea.setPromptText("Descrivi il problema...");
        ...
        dialog.setResultConverter(dialogButton -> {
            if (dialogButton == segnalaButtonType) {
                String note = noteArea.getText().trim();
                if (note.isEmpty()) {
                    showAlert("Errore", "Inserire una descrizione del problema");
                    return null;
                }
                return note;
            }
        ...
        dialog.showAndWait().ifPresent(note -> {
            try {
                compitoController.segnalaProblema(selectedCompito, note);
    ...
    private void loadMieiCompiti() {
        try {
            ...
            compitiTable.refresh(); // Forza il refresh della tabella
        } catch (Exception e) {
