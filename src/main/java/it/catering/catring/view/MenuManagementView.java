...
private HBox createActionButtons() {
    ...
    Button publishButton = new Button("📤 Pubblica in Bacheca");
...
private void pubblicaInBacheca() {
    ...
    if (selectedMenu.getSezioni().isEmpty()) {
        showAlert("Errore", "Non è possibile pubblicare un menu vuoto. Aggiungi almeno una sezione con delle ricette.");
        return;
    }
    ...
    confirmAlert.setContentText("Il menu diventerà visibile a tutti gli utenti del sistema nella bacheca pubblica. " +
                                "Una volta pubblicato, il menu non potrà più essere modificato.");
    ...
    confirmAlert.showAndWait().ifPresent(response -> {
        if (response == ButtonType.OK) {
            try {
                ...
                updateMenuDetails();
                showAlert("Successo", "🎉 Menu pubblicato con successo nella bacheca pubblica!\n\n" +
                         "Il tuo menu è ora visibile a tutti gli utenti e può essere utilizzato per gli eventi.");
            } catch (Exception e) {
        ...
        if (choice != cancelButton) {
            try {
                ...
                String content = String.format(
                    "📁 File salvato come: %s\n\n" +
                    "📂 Posizione: %s\n\n" +
                    "💡 Suggerimento: Puoi trovare il file nella cartella 'exports' " +
...
private void showEditMenuDialog() {
    ...
    // Se il menu ha già delle caratteristiche, le mostra come testo
    StringBuilder existingNotes = new StringBuilder();
    if (selectedMenu.getDescrizione() != null && !selectedMenu.getDescrizione().trim().isEmpty()) {
        existingNotes.append(selectedMenu.getDescrizione()).append("\n\n");
    }
    
    if (selectedMenu.isCuocoRichiesto()) existingNotes.append("• Cuoco richiesto durante il servizio\n");
    if (selectedMenu.isSoloPiattiFreddi()) existingNotes.append("• Solo piatti freddi\n");
    if (selectedMenu.isCucinaRichiesta()) existingNotes.append("• Cucina richiesta nella sede\n");
    if (selectedMenu.isAdeguatoBuffet()) existingNotes.append("• Adeguato per buffet\n");
    if (selectedMenu.isFingerFood()) existingNotes.append("• Finger food\n");
