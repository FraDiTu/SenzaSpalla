// File: src/main/java/it/catering/catring/view/CompitoManagementView.java
...
public class CompitoManagementView {
    ...
    private HBox createControlsBox() {
        ...
        ordinamentoComboBox.setValue("Menu/Ricetta");
        ordinamentoComboBox.setOnAction(e -> {
            ...
            if (selected != null) {
                ...
                try {
                    ...
                    showAlert("Errore", "Errore nell'ordinamento: " + ex.getMessage());
                }
            }
        });
    ...
    private void showAssignCompitoDialog() {
        ...
        tipoCombo.setItems(FXCollections.observableArrayList("Ricetta", "Preparazione"));
        tipoCombo.setValue("Ricetta");
        
        // ComboBox per ricette/preparazioni
        ComboBox<Preparazione> itemCombo = new ComboBox<>();
        itemCombo.setConverter(new javafx.util.StringConverter<Preparazione>() {
            @Override
            public String toString(Preparazione prep) {
                return prep != null ? prep.getNome() : "";
            }
            
            @Override
            public Preparazione fromString(String string) {
                return null;
            }
        });
        
        // Aggiorna il combo quando cambia il tipo
        tipoCombo.setOnAction(e -> {
            String tipo = tipoCombo.getValue();
            if ("Ricetta".equals(tipo)) {
                List<Ricetta> ricette = compitoController.getRicetteDisponibili();
                itemCombo.setItems(FXCollections.observableArrayList(ricette.stream()
                    .map(r -> (Preparazione) r).toList()));
            } else {
                List<Preparazione> preparazioni = compitoController.getPreparazioniDisponibili();
                itemCombo.setItems(FXCollections.observableArrayList(preparazioni));
            }
        });
        
        // Inizializza con ricette
        tipoCombo.fireEvent(new javafx.event.ActionEvent());
        ...
        grid.add(new Label("Ricetta/Preparazione:"), 0, 1);
        grid.add(itemCombo, 1, 1);
        grid.add(new Label("Cuoco:"), 0, 2);
        grid.add(cuocoCombo, 1, 2);
        grid.add(new Label("Tempo (min):"), 0, 3);
        grid.add(tempoField, 1, 3);
        grid.add(new Label("Quantità:"), 0, 4);
        grid.add(quantitaField, 1, 4);
        ...
        dialog.setResultConverter(dialogButton -> {
            if (dialogButton == assignButtonType) {
                try {
                    Preparazione item = itemCombo.getValue();
                    Cuoco cuoco = (Cuoco) cuocoCombo.getValue();
                    int tempo = Integer.parseInt(tempoField.getText());
                    double quantita = Double.parseDouble(quantitaField.getText());
                    
                    if (item == null || cuoco == null) {
                        showAlert("Errore", "Seleziona tutti i campi obbligatori");
                        return null;
                    }
                    
                    Compito compito;
                    if (item instanceof Ricetta ricetta) {
                        compito = compitoController.assegnaCompitoRicetta(ricetta, cuoco, tempo, quantita);
                    } else {
                        compito = compitoController.assegnaCompitoPreparazione(item, cuoco, tempo, quantita);
                    }
        ...
        dialog.showAndWait().ifPresent(compito -> {
            loadCompiti();
    ...
    private void showModifyCompitoDialog() {
        ...
        dialog.showAndWait().ifPresent(result -> {
            if (result) {
                loadCompiti();
                showAlert("Successo", "Compito modificato con successo!");
            }
        });
    }
