...
public class TestEventController {
    ...
    public void testCreateEvent_InvalidParameters() {
        ...
        assertThrows(IllegalArgumentException.class, () -> {
            eventController.createEvent(null, "Test Event", "Location", today.plusDays(1), 50);
        }, "Dovrebbe lanciare eccezione per organizer ID null");
        
        // Nome vuoto
        assertThrows(IllegalArgumentException.class, () -> {
            eventController.createEvent("ORG_001", "", "Location", today.plusDays(1), 50);
        }, "Dovrebbe lanciare eccezione per nome vuoto");
        
        // Location vuota
        assertThrows(IllegalArgumentException.class, () -> {
            eventController.createEvent("ORG_001", "Test Event", "", today.plusDays(1), 50);
        }, "Dovrebbe lanciare eccezione per location vuota");
        
        // Data nel passato
        assertThrows(IllegalArgumentException.class, () -> {
            eventController.createEvent("ORG_001", "Test Event", "Location", yesterday, 50);
        }, "Dovrebbe lanciare eccezione per data nel passato");
        
        // Numero ospiti negativo
        assertThrows(IllegalArgumentException.class, () -> {
            eventController.createEvent("ORG_001", "Test Event", "Location", today.plusDays(1), -10);
    ...
    public void testGetEventsInDateRange() {
        ...
        String eventInRange = eventController.createEvent("ORG_001", "Event in range", 
                                                          "Location", start.plusDays(2), 50);
        String eventOutOfRange = eventController.createEvent("ORG_001", "Event out of range", 
                                                             "Location", end.plusDays(5), 50);
        
        List<Event> eventsInRange = eventController.getEventsInDateRange(start, end);
        
        assertTrue(eventsInRange.stream().anyMatch(e -> e.getId().equals(eventInRange)));
        assertFalse(eventsInRange.stream().anyMatch(e -> e.getId().equals(eventOutOfRange)));
    }
    ...
    @Test
    @DisplayName("Test verifica conflitti programmazione chef")
    public void testCheckChefScheduleConflicts() {
        LocalDate conflictDate = LocalDate.now().plusDays(5);
        
        String event1 = eventController.createEvent("ORG_001", "Event 1", 
                                                    "Location", conflictDate, 50);
        String event2 = eventController.createEvent("ORG_001", "Event 2", 
                                                    "Location", conflictDate.plusDays(1), 60);
        
        eventController.assignChefToEvent(event1, "CHF_001", "ORG_001");
        eventController.assignChefToEvent(event2, "CHF_001", "ORG_001");
        
        // Avvia solo il primo evento
        eventController.addServiceToEvent(event1, conflictDate, 
                                          LocalTime.of(12, 0), LocalTime.of(15, 0), 
                                          Service.ServiceType.LUNCH);
        eventController.startEvent(event1, "ORG_001");
        
        List<Event> conflicts = eventController.checkChefScheduleConflicts("CHF_001", conflictDate);
        
        assertEquals(1, conflicts.size());
        assertTrue(conflicts.stream().anyMatch(e -> e.getId().equals(event1)));
    }
}
